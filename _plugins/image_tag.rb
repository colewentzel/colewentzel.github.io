...
module Jekyll
  ...
  class Image < Liquid::Tag
    ...
    def render(context)
      ...
      # Apply regex to parse the markup
      markup = /^(?:(?<preset>[^\s.:\/]+)\s+)?(?<image_src>[^\s]+\.[a-zA-Z0-9]{3,4})\s*(?<html_attr>[\s\S]+)?$/.match(render_markup)

      # Handle invalid markup early
      unless markup
        raise "Image Tag can't read this tag. Try {% image [preset or WxH] path/to/img.jpg [attr=\"value\"] %}."
      end

      # Extract preset, if any
      preset_name = markup[:preset]
